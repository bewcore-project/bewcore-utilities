...
def get_args():
    ...
    parser.add_option("-a", "--algorithm", dest="algorithm", default="yespower", help="the PoW algorithm: [SHA256|yespower]")
...
def get_algorithm(options):
    supported_algorithms = ["SHA256", "yespower"]
...
def generate_hashes_from_block(data_block, algorithm):
    ...
    if algorithm == 'yespower':
        try:
            exec('import %s' % "bewcore_yespower")
        except ImportError:
            sys.exit("Cannot run bewcore_yespower algorithm: module bitweb_yespower not found")
        header_hash = bewcore_yespower.getPoWHash(data_block)[::-1]
